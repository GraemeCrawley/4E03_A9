...
while (t<simtime)
    ...
    if (i==1) %event is an arrival
        ...
        % Only update JSQ after a set period of time (time_thresh)
        if((t - prevTime) > time_thresh)
            % The below code was taken from Sean de Wolskis answer here: https://www.mathworks.com/matlabcentral/answers/30815-efficient-sort-function-with-random-tie-breaking-rule
            % It's used to get the minimum queue length while breaking ties randomly
            [v,idx] = sort(q);
            idxc = cumsum([1 logical(diff(v))]);
            idx = cell2mat(accumarray(idxc',idx',[],@(q){q(randperm(numel(q)))}));
            JSQ = idx(1);
            prevTime = t;
        end
        if (JSQ==1)
